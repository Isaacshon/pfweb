...
export default function CommunityPage() {
  ...
  // Real Name Input (Simulated Auth)
  const currentUserName = "Isaac Shon" // In reality, this would come from user profile
  ...
  const sharePost = (post: any) => {
    ...
    if (navigator.share) {
      ...
      showNotify("Post link copied to clipboard!")
    }
  }

  // --- Long Press Logic for Picker ---
  ...
  const endLongPress = (postId: number, e: React.MouseEvent | React.TouchEvent) => {
      ...
      <div className="fixed inset-0 z-[50] flex flex-col overflow-hidden animate-in fade-in duration-700 bg-black font-pretendard">
        <div className="flex-1 flex overflow-hidden pb-24"> 
          ...
          >
            ...
            <h2 className="text-2xl md:text-5xl font-extralight tracking-[0.1em] md:tracking-[0.25em] text-white transition-all duration-1000">MEDITATION</h2>
          </button>
          <button 
          ...
          >
            ...
            <h2 className="text-2xl md:text-5xl font-extralight tracking-[0.1em] md:tracking-[0.25em] text-white transition-all duration-1000">PRAYER</h2>
          </button>
    ...
    <div className={`min-h-screen ${bgColor} ${textColor} pb-40 transition-colors duration-500 font-pretendard relative no-scrollbar overflow-x-hidden`}>
      ...
      <header className="px-6 pt-16 pb-4 flex items-center justify-between sticky top-0 z-40 bg-inherit/80 backdrop-blur-md border-b border-zinc-500/10">
        <div className="flex items-center gap-4">
          <button onClick={() => setView('selection')} className="w-10 h-10 rounded-full flex items-center justify-center bg-zinc-500/10"><span className="material-icons text-xl">arrow_back</span></button>
        ...
        {filteredPosts.map((p) => {
          ...
          return (
            <div key={p.id} className="flex flex-col border-b border-zinc-500/5">
              ...
              <div className="px-6 py-4 flex items-center justify-between relative">
                ...
                <div className="relative">
                  <button 
                    ...
                    onMouseUp={(e) => endLongPress(p.id, e)}
                    onTouchStart={() => startLongPress(p.id)}
                    onTouchEnd={(e) => endLongPress(p.id, e)}
                    className={`px-6 py-2 rounded-full text-[10px] font-black uppercase tracking-widest transition-all ${
                      userActiveReaction ? accentBg + ' text-white' : (isDarkMode ? 'bg-zinc-900' : 'bg-slate-50')
                    }`}
                  >
                  ...
                  {isPickerOpen && (
                    <div className={`absolute bottom-full right-0 mb-4 flex items-center gap-2 p-2 rounded-[24px] shadow-2xl border animate-in slide-in-from-bottom-2 duration-300 ${isDarkMode ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-slate-50'}`}>
                      {reactionTypes.map((rt) => (
                        <button 
                          key={rt.label}
                          onClick={() => handleReaction(p.id, rt.label)}
                          className="w-12 h-12 rounded-full flex flex-col items-center justify-center transition-all hover:scale-125 hover:bg-zinc-500/10"
                        >
                          <span className={`material-icons ${rt.color}`}>{rt.icon}</span>
                        </button>
                      ))}
                    </div>
                  )}
              ...
              {/* Collapsible Content & Real-name Comments */}
                  ...
                  <div className="space-y-6 pt-10 border-t border-zinc-500/10">
                      ...
                      {p.comments.map(c => (
                        <div key={c.id} className="flex flex-col gap-1">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-[9px] opacity-30">{c.date}</span>
                          </div>
                    ...
                    <div className="flex items-center gap-3 pt-4">
                      <input 
                        ...
                        onKeyDown={(e) => { if (e.key === 'Enter') { addComment(p.id, (e.target as any).value); (e.target as any).value = ''; } }}
                        className={`flex-1 h-14 px-6 rounded-2xl text-sm font-medium focus:ring-1 focus:ring-zinc-700 outline-none ${isDarkMode ? 'bg-zinc-900 text-white' : 'bg-white text-zinc-900 border'}`}
                      />
                    </div>
                  </div>
                </div>
      ...
      <button 
        ...
        className={`fixed bottom-28 right-6 w-14 h-14 rounded-full shadow-2xl flex items-center justify-center transition-all active:scale-90 z-40 ${accentBg} text-white`}
      >
        <span className="material-icons text-3xl">add</span>
      </button>

      {/* Simplified Write Modal */}
      {isWriteModalOpen && (
        <div className="fixed inset-0 z-[100] bg-black/60 backdrop-blur-md flex items-end animate-in fade-in duration-300">
          <div className={`w-full max-w-md mx-auto p-8 rounded-t-[48px] animate-in slide-in-from-bottom-10 duration-500 ${isDarkMode ? 'bg-zinc-900' : 'bg-white'}`}>
            <div className="flex justify-between items-center mb-10">
              <h2 className="text-xl font-black">Write Post</h2>
              <button onClick={() => setIsWriteModalOpen(false)} className="material-icons opacity-40">close</button>
            </div>
            ...
            <button 
              onClick={() => { setIsWriteModalOpen(false); showNotify("Post published successfully!"); }}
              className={`w-full py-5 rounded-3xl font-black uppercase tracking-widest text-xs ${accentBg} text-white shadow-xl`}
            >
              Post Now
            </button>
          </div>
